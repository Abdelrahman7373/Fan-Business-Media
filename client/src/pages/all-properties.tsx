...
import { Typography, Stack, Box, TextField, Select, MenuItem } from "@pankod/refine-mui";
...
const AllProperties = () => {
  ...
  const currentFillterValues = useMemo(() => {
          ...
          <Box mb={2} mt={3} display="flex" width="84%" justifyContent="space-between" flexWrap="wrap">
            <Box display="flex" gap={2} flexWrap="wrap" mb={{ xs: "20px", sm: 0 }}>
              ...
              <TextField 
                ...
                value={currentFillterValues.title}
                onChange={(e) => {
                  setFilters([
                    {
                      field: "title",
              ...
              <Select variant="outlined" color="info" displayEmpty required inputProps={{ "aria-label": "Without label" }} defaultValue="" value={currentFillterValues.propertyType} onChange={(e) => {
                setFilters([
                  {
                    field: "propertyType",
                    operator: "eq",
                ...
                {[ "Apartment", "Villa", "Farmhouse", "Condos", "Townhouse", "Duplex", "Studio", "Chalet", ].map((type) => (
                  <MenuItem key={type} value={type.toLocaleLowerCase()}>{type}</MenuItem>
                ))}
              </Select>
            </Box>
          </Box>
